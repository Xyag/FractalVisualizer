...
namespace FractalVisualizer
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            double xMin = -2;//Convert.ToDouble(args[0]);
            double yMin = -1.5;//Convert.ToDouble(args[1]);
            double xMax = 1;//Convert.ToDouble(args[2]);
            double yMax = 1.5;//Convert.ToDouble(args[3]);
            string imagePath = @"C:\Users\Jim\Pictures\Fractals\";//@"C:\Users\Public\Pictures\Mandelbrot\debug\";//args[4];

            Bitmap b = new Bitmap(720,720);
            int maxIterations = 100;

            Mandelbrot man = new Mandelbrot(graphColors);
            //man.Graph(new Complex(xMin,yMin), new Complex(xMax,yMax), maxIterations, ref b,2);
            ...
            Animator.AnimatePowerThreaded(man, new Complex(-2,-1.5), new Complex(1,1.5), 0, 10, 20, 4, 900);
        }
